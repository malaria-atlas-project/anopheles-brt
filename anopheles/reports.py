...
"""
sampleperiod_subq = session.query(SamplePeriod.anopheline_id, func.count(func.distinct(SamplePeriod.site_id)).label('site_count'), func.count('*').label('sampleperiod_count')).group_by(SamplePeriod.anopheline_id).subquery()

q = session.query(Anopheline.name, func.coalesce(sampleperiod_subq.c.sampleperiod_count, 0), func.coalesce(sampleperiod_subq.c.site_count,0))
